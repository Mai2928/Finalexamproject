...
Class Master extends DBConnection {
	...
	function register(){
		...
		$check = $this->conn->query("SELECT * FROM `user` where `email` = '{$email}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
	...
	function update_account(){
		...
		$check = $this->conn->query("SELECT * FROM `user`  where `email`='{$email}' and `id` != $id ")->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Email already taken.";
			return json_encode($resp);
		...
		$save = $this->conn->query("UPDATE `user` set $data where id = $id ");
		if($save){
			foreach($_POST as $k =>$v){
				if($k != 'cpassword')
				$this->settings->set_userdata($k,$v);
...
function update_myaccount(){
	...
	$check = $this->conn->query("SELECT * FROM `admin`  where `email`='{$email}' and `id` != $id ")->num_rows;
	if($check > 0){
		$resp['status'] = 'failed';
		$resp['msg'] = "Email sudah ada.";
		return json_encode($resp);
		exit;
	}
	foreach($_POST as $k =>$v){
	...
	$save = $this->conn->query("UPDATE `admin` set $data where id = $id ");
	if($save){
		foreach($_POST as $k =>$v){
			if($k != 'cpassword')
			$this->settings->set_userdata($k,$v);
